...
本 Skill 只学习适合长期保存的 Codex 行为偏好，不导出会话、不管理一般会话，也不生成工作流 Skill。它只把真实的用户输入与 Assistant 最终回复作为判断材料，不把系统注入、推理过程、工具输出或子代理内容误当成用户偏好。写入时只修改自己的托管区块，保留 `AGENTS.md` 其余内容；项目范围只来自可信会话元数据确定的根目录；遇到损坏的标记或符号链接目标时会拒绝写入。原始会话历史始终只读。
