...
- 挖掘 Codex 的活跃及归档会话历史，从用户的纠正、不满、验收标准、沟通偏好和安全边界中提炼可长期复用的规则。
- 根据适用范围将规则写入对应项目或全局 `AGENTS.md`；项目规则已存在时，相关历史仍会参与跨项目聚合，不会因为项目内无需修改而丢失全局价值。
...
Suggest 会完成历史挖掘和规则判断，但只向用户展示带编号的建议、原因与完整 diff，不会立即修改目标 `AGENTS.md`。用户可以自然语言说明接受哪些编号，也可以编辑或拒绝其中任何一项；只有明确接受的建议才会写入。首次使用以及需要人工把关的场景应采用此模式。

### Auto（需明确授权）

Auto 用于无需逐项确认的持续维护，只有用户明确启用后才会运行。它只自动应用高置信度、低过度约束风险的规则；证据不足、适用范围不清或风险较高的内容不会被强行写入。对于 Git 项目，成功写入后可按用户保存的偏好只提交目标 `AGENTS.md`，不会夹带仓库中的其他改动。Auto 不会由 Suggest 自动升级而来，也不会复用旧的 Suggest 结果。

## 安全边界

本 Skill 只把真实的用户输入与 Assistant 最终回复作为判断材料，不把系统注入、推理过程、工具输出或子代理内容误当成用户偏好。它只写入自己的托管区块，保留 `AGENTS.md` 其余内容；项目范围只来自可信会话元数据确定的根目录；遇到损坏的标记或符号链接目标时会拒绝写入。原始会话历史始终只读。
